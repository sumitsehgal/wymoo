...
function caseinfo() {
	...
	if (!empty($this->data)) {
		if(!empty($result) && $result[$this->modelClass]['is_submited']==1){
			$this->Session->setFlash(__('Your case has already been submitted and is no longer editable. Use Notifications tab to add new data.', true), 'error');
			if ($this->referer() != '/') {
				$this->redirect($this->referer());
			} else {
				$this->redirect(array('action' => 'casetracker'));
...
$this->layout="default";
	...
	if (!empty($this->data)) {
		...
		if ($this->{$this->modelClass}->validates() && $this->{$this->modelClass}->User->validates()) {
			...
			$duedate =  explode('-',$this->data[$this->modelClass]['subject_dob']);
			$duedate1 =  explode('-',$this->data[$this->modelClass]['subject_dob1']);

			if(count($duedate)==3 && $this->data[$this->modelClass]['subject_dob1']!='' && (isset($duedate1[2]) && $duedate1[2]!=date('Y'))){
				$this->data[$this->modelClass]['subject_dob'] =  mktime(23,59,59,$duedate[1]*1,$duedate[0]*1,$duedate[2]);	
			} else {
				$this->data[$this->modelClass]['subject_dob'] = 0;
			}
	...
	if (!empty($this->data)) {
		...
		if ($this->data[$this->modelClass]['notify'] == 1) {
			...
			if ($this->{$this->modelClass}->validates()) {
				...
				$site = Configure::read('sites_id.'.$result['CaseTable']['site_id'].'.site');
				$default_email = $sites[$site]['default_email']['email'];
				$noreply_email = $sites[$site]['noreply_email']['email'];
				$title = $sites[$site]['title'];

				$this->_sendMail($result[$this->modelClass]['client_email'], $title.' <'.$default_email.'>', $noreply_email, 'Your case has been updated' ,'notification',array('result' => $result ),'', 'html' );	

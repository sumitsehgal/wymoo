 ...
 */
	...
	public function view ($model, $id, $edit = false, $delete = false,$width='450px') {
		...
		if($edit){
			if($delete){
					$select = '<select id="' . $model . $id . '" data-placeholder="Click on Add to attach files" style="width: '.$width.';display:none;"  class="chzn-select" multiple="multiple" name="data[' . $model . '][' . $id . '][]">' . $select . "</select>"; 

			} else {
			$select = '<select id="' . $model . $id . '" data-placeholder="Click on Add to attach files" style="width: '.$width.';display:none;"  disabled="disabled" class="chzn-select" multiple="multiple" name="data[' . $model . '][' . $id . '][]">' . $select . "</select>"; 
		   }
		 return $select;
		}
	...
	public function edit ($model, $id,$btn= 'Attach File',$allowExt= array(),$maxFiles=5,$delete=true,$width='450px') {
		...
		$downloadurl 	= $this->Html->url(array('plugin' => 'ajax_multi_upload', 'controller' => 'uploads', 'action' => 'download'));
		$str1 = $this->view ($model, $id, true,$delete,$width);
		$template = '<div class="qq-uploader"><div class="qq-upload-drop-area well"><span>Drop files here to attach</span></div><div class="qq-upload-button">'.$btn.'</div><ul class="qq-upload-list"></ul></div>';
		$str = '';
		...
		$str .= <<<END
			...
			<script>    
				...
				$(function(){
					...
					$('#'+ '$model' + '$id' ).chosen({disabled:true}).change(function(){
						if('$deleteurl' !=''){
							...
							$('#'+ '$model' + '$id').find("option").attr("selected", true);
